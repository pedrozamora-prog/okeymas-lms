...
export async function GET() {
  ...
  const avgSimScore = simulationStats.length > 0
    ? Math.round(simulationStats.reduce((s: number, a: { score: number }) => s + (a.score ?? 0), 0) / simulationStats.length)
    : null;
  ...
  // Build stats context for Gemini
  const statsContext = `
...
IMPORTANTE: Devuelve ÚNICAMENTE un JSON array. Sin texto adicional. Sin markdown.
